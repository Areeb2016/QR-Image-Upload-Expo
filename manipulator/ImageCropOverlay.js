...
class ImageCropOverlay extends React.Component {
  ...
  render() {
    ...
    style.top =
      initialTop +
      (draggingTL || draggingTM || draggingTR || draggingMM ? offsetTop : 0);
    style.left =
      initialLeft +
      (draggingTL || draggingML || draggingBL || draggingMM ? offsetLeft : 0);
    style.width =
      initialWidth +
  ...
  handlePanResponderEnd = (e, gestureState) => {
    ...
    if (ratio && ratio.height && ratio.width) {
      if (
        state.initialWidth * ratio.width >
        state.initialHeight * ratio.height
      ) {
